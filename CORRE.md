...
**Corolário — `'sistema'` é tipo, não ator.** Ninguém prova ser o sistema, porque não existe credencial de sistema: quem o declara está afirmando algo que **não se verifica**. Por isso `'sistema'` só vale em **caminho interno** — o varredor de prazos hoje, o webhook do gateway na Etapa 7 — e **nada que receba entrada de fora repassa essa marca**. *Origem, 2026-08-10:* o evento do sistema tem autor nulo, então a conferência de autor comparava `null` com `null` e dava certo para qualquer chamador sem id.
