...
**Corolário — autorização vem ANTES do atalho.** Todo caminho que devolve resultado sem passar pela validação — replay de idempotência, cache, resposta repetida — tem que conferir o autor **antes de responder**, não depois. Enquanto o atalho vem primeiro, quem acerta a chave recebe a resposta **sem provar nada**. *Origem, 2026-08-10:* foi assim que a chave de criação virou chave-mestra entre lojistas, e assim que o evento do `sistema` era entregue a qualquer chamador — a correção da flag de origem, sozinha, não consertou nada, porque o replay respondia antes de a flag ser lida.

**Corolário — `'sistema'` é tipo, não ator.** Ninguém prova ser o sistema, porque não existe credencial de sistema: quem o declara está afirmando algo que **não se verifica**. Por isso **todo caminho interno que age como sistema precisa de origem verificável — nunca de omissão de id**. Vale para o varredor de prazos hoje, para o webhook do gateway na Etapa 7 e **para todo processo de fundo que qualquer etapa criar**; nada que receba entrada de fora repassa essa marca. *Origem, 2026-08-10:* o evento do sistema tem autor nulo, então a conferência de autor comparava `null` com `null` e dava certo para qualquer chamador sem id. E em `clientes.js` era pior — bastava **omitir** o lojista para o evento sair com `autor_tipo: 'sistema'`: **a ausência de um campo virava a autoridade mais alta do sistema.**

**Existência não é aptidão.** Conferir que o ator **existe** não prova que ele **pode agir**. Bloqueado, suspenso e pendente existem. Toda porta que confere identidade confere também **situação**, e no mesmo lugar — senão o bloqueio, que é a única ferramenta de expulsão da plataforma (seção 13), vale só nas portas em que alguém lembrou. *Origem, 2026-08-10:* `exigeAutorReal` perguntava só `SELECT id`, e "existe" virou "pode": um **motoboy bloqueado por roubo** continuava aceitando corrida e confirmando coleta, com a mercadoria de terceiro na mão dele. O resto do projeto já tratava assim — a sessão não nasce para conta bloqueada e é revalidada contra a conta viva a cada requisição. Só o motor que move mercadoria não.

**Proteção que ninguém desenhou não é proteção.** Quando descobrir que algo está seguro, responda **quem o protege**. Se a resposta for "aconteceu de funcionar", trate como **aberto** — o acidente que fecha hoje some no dia em que a peça de cima mudar, e ninguém vai ligar as duas coisas. *Origem, 2026-08-10:* a rota de estorno do painel passava o pool **sem cidade**; parecia fechada porque o RLS a cegava, mas ninguém a havia protegido — ela simplesmente **nunca funcionou**, e no dia em que a política caísse leria corrida de qualquer cidade.
...
- **Achado de auditoria vai para o repositório no momento em que é CONFIRMADO por reprodução, não quando a etapa fecha.** Entre a confirmação e o fim da etapa pode haver troca de sessão, e **defeito confirmado que só existe em conversa é defeito perdido**. O mesmo vale para **decisão tomada em sessão: entra no documento no ato**. *(Regra de 2026-08-10: cinco achados da auditoria da Lei 11 — um deles um UUID inventado cancelando corrida com mercadoria na rua — existiam só no relatório de conversa quando a sessão virou.)*
- **Contexto no fim é motivo válido para PARAR antes do controle negativo — nunca para correr por cima dele.** Fechar a etapa com a bateria verde e a sabotagem por fazer é entregar meia prova, e meia prova pesa como prova inteira na cabeça de quem lê o relatório. Parar, dizer o que falta e retomar em sessão nova custa uma sessão; correr por cima custa o falso verde que a Lei 8 existe para pegar. *(Regra de 2026-08-10, na rodada da Lei 11.)*
- **Teste que depende do defeito para passar é pior que teste ausente: ele defende o defeito.** Teste ausente deixa um buraco visível; teste que se apoia no defeito produz **verde**, e verde é a única coisa que ninguém vai reabrir. Quando corrigir a regra quebra a bateria, a leitura certa é **"o auxiliar de teste estava errado"**, nunca "a correção está severa demais" — e o auxiliar se conserta ANTES da correção, senão a pressão de ter cinquenta testes vermelhos empurra para afrouxar exatamente a trava que se quer instalar. *(Origem: 2026-08-10. Os 232 testes da bateria geravam `autorId: randomUUID()` — autor que não existe — e por isso passavam. Exigir autor real quebrou sete deles de uma vez. O gerador de atores foi trocado primeiro, de propósito, e só depois a trava entrou.)*
- **Idempotência sem teste de retentativa IDÊNTICA não está testada.** Testar que a mesma chave com dados **diferentes** é recusada prova metade; a outra metade — que a mesma chave com dados **iguais** é operação nula — é o caso canônico da Lei 5 ("a operação chegou, a resposta se perdeu") e é o que quebra na prática. Toda função com chave de idempotência precisa das duas. *(Origem: 2026-08-10. Uma conferência de dados nova comparava o pedido recalculado com o gravado; em `trocaAparelho` um dos campos é DERIVADO da projeção, que na retentativa já mudou, e a retentativa byte a byte idêntica passou a devolver 409. A bateria não viu porque só existia o teste da metade.)*
- **Idempotência compara PEDIDO com PEDIDO, já normalizado — nunca pedido com estado.** Campo derivado do servidor não entra na comparação da chave, e valor cru não se compara com valor gravado: o gravado passou por normalização e nunca vai casar.
...
| 6 | Despacho: cascata, timer de 30s, regras de recusa | **⚠️ PRIMEIRO ITEM OBRIGATÓRIO, antes da cascata e antes de qualquer outra coisa: o VÍNCULO DO MOTOBOY COM A CORRIDA** (`corridas.motoboy_id` + chave composta). **Dívida herdada da Lei 11, 2026-08-10:** hoje qualquer motoboy declara chegada e confirma entrega de corrida alheia; a metade do lojista já foi fechada, e a do motoboy não deu porque a coluna nasce aqui. **Não é sugestão de ordem, é pré-requisito da etapa.** · 50 aparelhos disputando a mesma corrida resultam em exatamente 1 aceite. Cascata de 5 min sem aceite leva a Sem motoboy **sem nenhum movimento de dinheiro**. 4 recusas seguidas → offline por 15 min. Teto de 2 corridas ativas. **⬅ Critério herdado da Etapa 4:** corrida cujo **motoboy** seja de outra cidade é recusada **pelo banco** — não coube na Etapa 4 porque `corridas` só ganha coluna de motoboy aqui | não iniciada |
...
24. **⚠️ RISCO ACEITO E DECLARADO — enumeração de CPF e telefone no cadastro.** O cadastro responde **409** quando o CPF ou o telefone já existe, e isso **revela existência**: qualquer um descobre se um CPF é motoboy do Corre, ou se um telefone é lojista. **O dano real:** um concorrente monta a lista de quem roda com a gente e assedia; alguém cruza um vazamento de CPFs de outro lugar com essa resposta e sabe quem entrega em Sobral. Não expõe nome, endereço nem dinheiro — expõe **filiação**.
    **Por que fica aberto:** fechar exige que o cadastro **pare de emitir sessão para todo mundo** (a diferença entre a resposta nova e a repetida É a enumeração), e isso põe login obrigatório na largada do motoboy — atrito onde menos se pode ter. *Tentado e revertido em 2026-08-10; ver `HISTORICO.md`, decisão 160.*
    **Condição de revisão, para não virar esquecimento:** volta à mesa **quando a base crescer** ou **ao primeiro sinal de raspagem** (picos de cadastro recusado vindos do mesmo IP ou faixa), junto com a decisão de login obrigatório.
25. **Tempo base de coleta por LOJA, em vez de por cidade** (seção 8). **Sem etapa dona, de propósito.** Existe loja que separa o pedido em dois minutos e loja que leva quinze, e um dia a operação vai querer distinguir. Quando quiser, isso é **uma versão nova da tabela** — que é exatamente para o que o versionamento existe, e por isso não se constrói nada agora. Construir hoje o campo que ninguém preenche seria "deixar preparado", que é proibido
